...
export function AdminDashboard({ onLogout }) {
  ...
  return (
    <div className="min-h-screen bg-nutribowl-beige/40 pb-16 font-sans relative">
      
      {/* Top Navbar */}
      <nav className="bg-white border-b border-nutribowl-border/40 sticky top-0 z-40 shadow-sm">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        ...
        {stats && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
            ...
            <div className="bg-white p-6 rounded-3xl border border-nutribowl-border/40 shadow-sm flex items-center justify-between">
              <div>
                <p className="text-xs font-bold text-nutribowl-muted uppercase tracking-wider">Active Orders</p>
                <h3 className="text-2xl font-black text-nutribowl-brown mt-1">{stats.summary.activeOrders}</h3>
              </div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-nutribowl-border/40 shadow-sm flex items-center justify-between">
              <div>
                <p className="text-xs font-bold text-nutribowl-muted uppercase tracking-wider">Total Orders</p>
                <h3 className="text-2xl font-black text-nutribowl-brown mt-1">{stats.summary.totalOrders}</h3>
              </div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-nutribowl-border/40 shadow-sm flex items-center justify-between">
              <div>
                <p className="text-xs font-bold text-nutribowl-muted uppercase tracking-wider">Average Order Value</p>
                <h3 className="text-2xl font-black text-nutribowl-brown mt-1">{formatINR(stats.summary.averageOrderValue)}</h3>
              </div>
          ...
          <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-white p-6 rounded-[32px] border border-nutribowl-border/40 shadow-sm space-y-6">
                  ...
                  {Object.keys(stats.slotBreakdown).length === 0 ? (
                    <p className="text-sm text-nutribowl-muted text-center py-8">No slot data available yet.</p>
                  ) : (
                        ...
                        .map(([slot, count]) => {
                          ...
                          return (
                            <div key={slot} className="space-y-1">
                              <div className="flex justify-between text-xs font-bold text-nutribowl-brown">
                                <span className="flex items-center gap-1.5">
                                  <Clock size={12} className="text-nutribowl-orange" />
                                  {slot}
                                </span>
                                <span>{count} {count === 1 ? 'order' : 'orders'}</span>
                              </div>
                              <div className="w-full bg-nutribowl-beige/50 h-3 rounded-full overflow-hidden border border-nutribowl-border/20">
                                <div 
                                  className="bg-nutribowl-orange h-full rounded-full" 
                                  style={{ width: `${percent}%` }}
                               />
                              </div>
                            </div>
                ...
                <div className="bg-white p-6 rounded-[32px] border border-nutribowl-border/40 shadow-sm space-y-6">
                  ...
                  {Object.keys(stats.popularBases).length === 0 ? (
                    <p className="text-sm text-nutribowl-muted text-center py-8">No item sales data yet.</p>
                  ) : (
                ...
                <div className="bg-white p-6 rounded-[32px] border border-nutribowl-border/40 shadow-sm space-y-6">
                  ...
                  {Object.keys(stats.popularAddons).length === 0 ? (
                    <p className="text-sm text-nutribowl-muted text-center py-8">No topping sales data yet.</p>
                  ) : (
                    <div className="space-y-3.5">
                      {Object.entries(stats.popularAddons)
                        .sort((a, b) => b[1] - a[1])
                        .map(([addon, qty], idx) => (
                          <div key={addon} className="flex justify-between items-center text-xs font-bold py-2 border-b border-nutribowl-border/10 last:border-0">
                            <span className="flex items-center gap-2">
                              <span className="w-5 h-5 rounded-lg bg-nutribowl-beige flex items-center justify-center text-[10px] text-nutribowl-orange font-black">
                                #{idx + 1}
                              </span>
                              <span className="text-nutribowl-brown font-black">{addon}</span>
                            </span>
                            <span className="bg-nutribowl-lightOrange text-nutribowl-orange px-2.5 py-1 rounded-full text-[10px]">
                              {qty} selected
                            </span>
                          </div>
                        ))}
            ...
            {activeTab === 'menu' && (
              <div className="space-y-6">
                ...
                {/* Menu Action bar */}
                <div className="bg-white p-4 rounded-3xl border border-nutribowl-border/40 shadow-sm flex flex-col sm:flex-row gap-4 justify-between items-center">
                  {/* Search Input */}
                  ...
                  <>
                    ...
                    {menuSubTab === 'bases' && (
                      filteredMenuBases.length === 0 ? (
                        <div className="bg-white rounded-3xl p-16 border border-nutribowl-border/40 text-center">
                          <Layers className="mx-auto text-nutribowl-muted mb-4 animate-pulse" size={36} />
                          <h4 className="font-extrabold text-nutribowl-brown">No base items found</h4>
                          <p className="text-xs text-nutribowl-muted mt-1">Add your first breakfast oatmeal base or wheat bread slice!</p>
                        </div>
                      ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                          {filteredMenuBases.map((base) => (
                            <div 
                              key={base.id} 
                              className={`bg-white rounded-3xl border border-nutribowl-border/40 overflow-hidden shadow-sm flex flex-col justify-between transition-all hover:shadow-md ${
                                base.inStock === false ? 'opacity-70 bg-gray-50/50' : ''
                              }`}
                            >
                              <div className="relative h-44 bg-nutribowl-beige">
                                <div 
                              ...
                              <div className="p-5 flex-grow flex flex-col justify-between gap-4">
                                <div className="space-y-2">
                                  <div className="flex justify-between items-start gap-2">
                                    <h4 className="font-black text-nutribowl-brown text-sm sm:text-base leading-snug">{base.name}</h4>
                                    <span className="font-black text-nutribowl-orange text-base shrink-0">{formatINR(base.price)}</span>
                                  </div>
                                  <p className="text-xs text-nutribowl-muted leading-relaxed line-clamp-2">{base.desc || 'No description provided.'}</p>
                                  {base.tags && base.tags.length > 0 && (
                                    <div className="flex flex-wrap gap-1">
                                      {base.tags.map(t => (
                                        <span key={t} className="text-[8px] font-black uppercase bg-nutribowl-beige text-nutribowl-brown px-1.5 py-0.5 rounded border border-nutribowl-border/45">{t}</span>
                                      ))}
                                    </div>
                                  )}
                                </div>

                                <div className="flex items-center justify-between border-t border-nutribowl-border/20 pt-4 mt-1">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[11px] font-bold text-nutribowl-brown uppercase tracking-wider">
                    ...
                    {menuSubTab === 'combos' && (
                      filteredMenuCombos.length === 0 ? (
                        <div className="bg-white rounded-3xl p-16 border border-nutribowl-border/40 text-center">
                          <Sparkles className="mx-auto text-nutribowl-muted mb-4 animate-pulse" size={36} />
                          <h4 className="font-extrabold text-nutribowl-brown">No signature dishes found</h4>
                          <p className="text-xs text-nutribowl-muted mt-1">Create a popular oatmeal + toppings house combination!</p>
                        </div>
                      ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                          {filteredMenuCombos.map((combo) => (
                            ...
                            >
                              ...
                              <div className="p-5 flex-grow flex flex-col justify-between gap-4">
                                <div className="space-y-2">
                                  <div className="flex justify-between items-start gap-2">
                                    <h4 className="font-black text-nutribowl-brown text-sm sm:text-base leading-snug">{combo.name}</h4>
                                    <span className="font-black text-nutribowl-orange text-base shrink-0">{formatINR(combo.price)}</span>
                                  </div>
                                  <p className="text-xs text-nutribowl-orange font-bold uppercase">Base: {combo.base}</p>
                                  <p className="text-xs text-nutribowl-muted leading-relaxed">
                                    Toppings: {combo.addons && Array.isArray(combo.addons) ? combo.addons.join(' • ') : 'None'}
                                  </p>
                                  {combo.tag && (
                                    <span className="inline-block text-[8px] font-black uppercase bg-nutribowl-lightOrange text-nutribowl-orange px-2 py-0.5 rounded border border-nutribowl-orange/15">{combo.tag}</span>
                                  )}
                                </div>
                    ...
                    {menuSubTab === 'addons' && (
                      filteredMenuAddons.length === 0 ? (
                        <div className="bg-white rounded-3xl p-16 border border-nutribowl-border/40 text-center">
                          <Sparkles className="mx-auto text-nutribowl-muted mb-4 animate-pulse" size={36} />
                          <h4 className="font-extrabold text-nutribowl-brown">No addon toppings found</h4>
                          <p className="text-xs text-nutribowl-muted mt-1">Add raw organic seeds, berries, honey spreads or dry fruits!</p>
                        </div>
                      ) : (
                          ...
                          {["Spreads & Sweeteners", "Fresh Fruits", "Premium Nuts", "Healthy Seeds"].map((categoryName) => {
                            ...
                            return (
                              <div key={categoryName} className="space-y-4">
                                <div className="border-b border-nutribowl-border/30 pb-2 flex items-center gap-2">
                                  <span className="w-1.5 h-4 bg-nutribowl-orange rounded-full" />
                                  <h4 className="font-black text-xs sm:text-sm text-nutribowl-brown uppercase tracking-wider">{categoryName}</h4>
                                </div>
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                                  {categoryAddons.map((addon) => (
                                    <div 
                                      key={addon.id}
                                      className={`bg-white rounded-2xl border border-nutribowl-border/40 p-4 shadow-sm flex flex-col justify-between gap-3 ${
                                        addon.inStock === false ? 'opacity-70 bg-gray-50/50' : ''
                                      }`}
                                    >
                                      <div className="flex gap-3">
                                        <div 
                                          className="w-12 h-12 rounded-xl bg-cover bg-center flex-shrink-0 bg-nutribowl-beige relative overflow-hidden"
                                        ...
                                        <div className="min-w-0 flex-grow">
                                          <h5 className="font-bold text-nutribowl-brown text-xs sm:text-sm truncate leading-snug">{addon.name}</h5>
                                          <span className="text-xs font-black text-nutribowl-orange block mt-0.5">{formatINR(addon.price)}</span>
                                        </div>
                                      </div>

                                      <div className="flex items-center justify-between border-t border-nutribowl-border/10 pt-3 mt-1 select-none">
                                        {/* Stock Switch */}
                                        <button
                                          type="button"
        ...
        <AnimatePresence>
          {alerts.map((alert) => (
            <motion.div
              ...
              className="bg-white/95 backdrop-blur-md border-l-4 border-l-nutribowl-orange border-nutribowl-border/40 p-5 rounded-2xl shadow-xl flex gap-3 pointer-events-auto relative overflow-hidden"
            >
              ...
              <div className="flex-grow pr-4">
                <span className="text-[10px] font-black uppercase text-nutribowl-orange tracking-wider flex items-center gap-1">
                  <Sparkles size={10} className="animate-pulse" />
                  New Order Received!
                </span>
                <h4 className="font-black text-nutribowl-brown text-sm leading-tight mt-0.5">{alert.order.id} ({formatINR(alert.order.totalPrice)})</h4>
                <p className="text-xs text-nutribowl-muted mt-1">Customer: <span className="font-bold">{alert.order.customer.name}</span></p>
                <p className="text-[10px] text-nutribowl-muted mt-0.5">Slot: {alert.order.customer.timeSlot}</p>
              </div>
              <button
                onClick={() => setAlerts(prev => prev.filter(a => a.id !== alert.id))}
                className="text-nutribowl-muted hover:text-nutribowl-brown hover:bg-nutribowl-beige/50 p-1.5 rounded-lg self-start transition-colors"
              >
                <X size={14} />
              </button>
            </motion.div>
      ...
      <AnimatePresence>
        {showMenuModal && (
          <div className="fixed inset-0 bg-nutribowl-brown/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div 
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="bg-white rounded-[32px] border border-nutribowl-border/40 shadow-2xl max-w-lg w-full overflow-hidden flex flex-col max-h-[90vh]"
            >
              {/* Modal Header */}
              <div className="p-6 border-b border-nutribowl-border/20 flex justify-between items-center bg-nutribowl-beige/25">
                <h3 className="font-black text-nutribowl-brown text-lg uppercase tracking-wide">
                  {editingItem ? 'Edit Menu Item' : 'Add New Menu Item'}
                </h3>
                <button 
                  onClick={() => setShowMenuModal(false)}
                  className="text-nutribowl-muted hover:text-nutribowl-brown p-1.5 rounded-xl hover:bg-nutribowl-beige/50 transition-colors"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <form onSubmit={handleMenuFormSubmit} className="p-6 overflow-y-auto space-y-4 text-sm flex-grow">
                ...
                <div>
                  <label className="block text-xs font-bold text-nutribowl-muted uppercase tracking-wider mb-1.5">Item Type</label>
                  <div className="flex flex-wrap gap-4">
                    <label className="flex items-center gap-2 cursor-pointer font-bold text-nutribowl-brown">
                      <input
                        type="radio"
                ...
                {menuFormData.type === 'addon' && (
                  <div>
                    <label className="block text-xs font-bold text-nutribowl-muted uppercase tracking-wider mb-1.5">Category</label>
                    <select
                ...
                {menuFormData.type === 'combo' && (
                  <div>
                    <label className="block text-xs font-bold text-nutribowl-muted uppercase tracking-wider mb-1.5">Highlight Tag</label>
                    <input
                      type="text"
                ...
                {menuFormData.type === 'combo' && (
                  <div>
                    <label className="block text-xs font-bold text-nutribowl-muted uppercase tracking-wider mb-1.5">Select Base Bowl / Bread</label>
                    <select
                ...
                {menuFormData.type === 'combo' && (
                  <div>
                    <label className="block text-xs font-bold text-nutribowl-muted uppercase tracking-wider mb-1.5">Select Addons & Toppings</label>
                    <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto p-3 border border-nutribowl-border rounded-xl bg-nutribowl-beige/10">
                      {menuAddons.map(addon => {
                        const isChecked = menuFormData.addons?.includes(addon.name);
                        return (
                          <label key={addon.id} className="flex items-center gap-2 cursor-pointer font-medium text-nutribowl-brown text-xs">
                            <input
                              type="checkbox"
                              checked={isChecked}
                              onChange={() => {
                ...
                {/* Name */}
                <div>
                  <label className="block text-xs font-bold text-nutribowl-muted uppercase tracking-wider mb-1.5">Item Name</label>
                  <input
                    type="text"
                    required
                ...
                {/* Price */}
                <div>
                  <label className="block text-xs font-bold text-nutribowl-muted uppercase tracking-wider mb-1.5">Price (INR)</label>
                  <input
                    type="number"
                    required
                    min="0"
                ...
                {/* Image URL */}
                <div>
                  <label className="block text-xs font-bold text-nutribowl-muted uppercase tracking-wider mb-1.5">Image URL</label>
                  <input
                    type="text"
                ...
                {/* Tags */}
                <div>
                  <label className="block text-xs font-bold text-nutribowl-muted uppercase tracking-wider mb-1.5">Tags (Comma separated)</label>
                  <input
                    type="text"
                ...
                {menuFormData.type === 'base' && (
                  <div>
                    <label className="block text-xs font-bold text-nutribowl-muted uppercase tracking-wider mb-1.5">Description</label>
                    <textarea
                      rows="3"
                ...
                {/* Submit Actions */}
                <div className="flex gap-3 pt-4 border-t border-nutribowl-border/20 mt-6 justify-end">
                  <button
                    type="button"
                    onClick={() => setShowMenuModal(false)}
                    className="px-5 py-2.5 border border-nutribowl-border hover:bg-nutribowl-beige/40 text-nutribowl-brown rounded-xl font-bold transition-colors"
                  >
                    Cancel
                  </button>
                  <button
